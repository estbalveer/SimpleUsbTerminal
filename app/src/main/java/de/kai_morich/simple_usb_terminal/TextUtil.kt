...
object TextUtil {
    ...
    fun toCaretString(s: CharSequence, keepNewline: Boolean, length: Int = s.length): CharSequence {
        var found = false
        for (pos in 0..<length) {
            if (s.get(pos).code < 32 && (!keepNewline || s.get(pos) != '\n')) {
                found = true
                break
            }
        }
        ...
        for (pos in 0..<length) if (s.get(pos).code < 32 && (!keepNewline || s.get(pos) != '\n')) {
            sb.append('^')
            sb.append((s.get(pos).code + 64).toChar())
            sb.setSpan(
